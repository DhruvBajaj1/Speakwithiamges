      ...
      {data.map((item) => {
        return (
          <div className="card homecard" key={item._id}>
            <h5><Link to={item.postedBy._id !== state._id?"/profile/"+item.postedBy._id:"/profile"}><span style={{fontWeight:"500", color:"black"}}>{item.postedBy.name}</span></Link> {item.postedBy._id==state._id
            ...
            <div className="card-content">
                ...
                item.comments.map(record=>{
                  return(
                    <h6 key={record._id}> <span style={{fontWeight:"500"}}>{record.postedBy.name} : </span>{record.text} 
                     {/* { <span>{state._id==record.postedBy._id
                     && <i className="material-icons" style={{float:"right"}} 
              ...
              <input type="text" placeholder="add a comment" style={{fontWeight:"500", color:"black"}}/>
              </form>
            </div>
          </div>
        );
